...
def load_vgg(sess, vgg_path):
    ...
    return input_img, keep_prob, layer_3, layer_4, layer_7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    L7_conv2d_1X1 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, strides=(1,1), padding="same" ,kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    output = tf.layers.conv2d_transpose(L7_conv2d_1X1, num_classes, 4, strides=(2,2), padding="same" ,kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    L4_conv2d_1X1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, strides=(1,1), padding="same" ,kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    output = tf.add(L4_conv2d_1X1, output)
    ...
    L3_conv2d_1X1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, strides=(1,1), padding="same" ,kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    output = tf.add(L3_conv2d_1X1, output)
    ...
    for epoch in range(epochs):   
        for image, label in get_batches_fn(batch_size):
            ...
            _, loss = sess.run([train_op, cross_entropy_loss],feed_dict = {input_image: image, 
                                                                           ...
                                                                           keep_prob:0.5 ,
                                                                           learning_rate: 9e-5 })  
            print("Epoch %i ->  Loss %.2f" % (epoch+1, loss))
            
            
    
tests.test_train_nn(train_nn)
print("Training NN -> OK")
...
def run():
    ...
    with tf.Session() as sess:
        ...
        input_img, keep_prob, layer_3, layer_4, layer_7 = load_vgg(sess, vgg_path)
        ...
        train_nn(sess, 25, 4, get_batches_fn, train_op, cross_entropy_loss, input_img, correct_label, keep_prob, learning_rate)
